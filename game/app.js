...
// 사용자가 무인도에 있는 도중에 실행할 함수.
function isIslandUser() {
    if ($userCount === 18 && $islandUserCount < 2) {
        $h1.innerHTML = `사용자는 현재 무인도에 있습니다.<br>앞으로 ${2-$islandUserCount}번 동안 주사위를 굴려도 움직일 수 없습니다.`;
        $islandUserCount++;
        return;
...
// 컴퓨터가 무인도에 있는 도중 실행할 함수.
function isIslandCom() {
    if ($comCount === 18 && $islandComCount < 2) {
        $h1.innerHTML = `컴퓨터는 현재 무인도에 있습니다.<br>앞으로 ${2-$islandComCount}번 주사위를 굴려도 움직일 수 없습니다.`;
        $islandComCount++;
        return;
...
// 사용자가 무인도에 도착하면 실행할 함수.
function userEventIsland() {
    // 만약 유저가 도착한 지점이 무인도이면 무인도에 도착해버렸네 ㅠㅠ 알려주기.
    if ($userCount === 18) {
        printMessage += `<br>앗! 무인도에 도착해버렸네요..<br>3회 움직임이 제한됩니다.`;
    }

    // 만약 유저가 도착한 지점이 '무인도 여행'칸이면 유를 무인도로 유배보내기
    if ($userCount === 34) { // arr[34]는 무인도 여행 벌칙이 걸린 td !!
        $userCount = 18; // arr[18]은 무인도 벌칙 td!!
        printMessage += `<br>앗! 무인도로 강제 여행을 떠나게 됐습니다..<br>3회 움직임이 제한됩니다.`;
    }
}

// 컴퓨터가 무인도에 도착하면 실행할 함수.
function comEventIsland() {
...
// 사용자가 이동할 함수. (아이콘과 생성 포함!)
function userGoBack($userCount, $userNum) {
...
// 컴퓨터가 이동할 함수. (아이콘과 생성 포함!)
function comGoBack($comCount, $comNum) {
...
function randomDice() {
    ...
    if ($userNum > $comNum) {
        ...
        isIslandUser($userCount, $userNum); // 사용자가 무인도인지 검증할 함수.

        removeUser(); // 이전 사용자 이모티콘 지우는 함수.

        userGoBack($userCount, $userNum); // 이벤트에 따라 Go/Back 처리 및 사용자 아이콘까지 표시할 함수.

        userEventIsland($userCount, $userNum); // 사용자가 무인도에 도착하면 실행할 함수.
        ...
        isIslandCom($comCount, $comNum);

        removeCom(); // 이전 컴퓨터 이모티콘 지우는 함수.

        // Go/Back 처리
        comGoBack($comCount, $comNum);

        comEventIsland($comCount, $comNum);
...
// 게임이 종료되면 (isFinish가 true가 되면) 새로고침할 것인지를 묻는 함수.
function reTry() {
    if (isFinish) {
...
$btn.addEventListener('click', e => {
    reTry(); // 게임이 종료된 상태이면 재시작할 것인지를 묻고 재시작하겠다고 하면 페이지를 새로고침할 함수.

    throwDice(); // 주사위 던지는 애니메이션 함수.

    randomDice(); // 주사위 값 랜덤 부여해주는 함수.
